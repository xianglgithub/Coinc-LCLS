...
import PiPiCo3GatedPlots, PiPiCo3GatedPlots1D
    ...
    def sum_under_2d_roi(self):
      #  print 'sum_2d i:',i
       ...
       # return np.sum(np.sum(arr))*scale_arr, coords_border_y_x1, coords_border_y_x2
        self.parent.PiPiCoUi.Rect3Yield.setText(self.roiName+"Yield:%0.1f" % roi_yield)
...
#        print self.XYG_ind1, self.XYG_ind2 
    ...
    def initialize_plots(self):
        ...
        self.plotWindow = PiPiCoGPlots3()   
        self.plot1dWindow = PiPiCoGPlots1D3()           
...
class PiPiCoGPlots1D3(QtGui.QMainWindow):
    def __init__(self, parent=None):

        super(PiPiCoGPlots1D3, self).__init__()
        
        
        self.PiPiCoGPlots1DUi = PiPiCo3GatedPlots1D.Ui_PiPiCo3GatedPlots1D()
...
class PiPiCoGPlots3(QtGui.QMainWindow):
    def __init__(self, parent=None):

        super(PiPiCoGPlots3, self).__init__()
        
        self.pos, self.colors = zip(*cmapToColormap(matplotlib.cm.jet))
        # Set the colormap
        self.clrmp =  pg.ColorMap(self.pos, self.colors)
        
        self.PiPiCoGPlotsUi = PiPiCo3GatedPlots.Ui_PiPiCo3GatedPlots()
        self.PiPiCoGPlotsUi.setupUi(self)
        
        self.PiPiCoItem = pg.PlotItem(title = 'PiPiCo3',labels={'bottom': ('T', 'ns'), 'left': ('T', 'ns')})
        self.PiPiCoItem.setAspectLocked(False)                      
        self.PiPiCoPlot = pg.ImageView(name='PiPiCo3', view=self.PiPiCoItem) 
