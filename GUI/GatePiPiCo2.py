...
import PiPiCo2GatedPlots, PiPiCo2GatedPlots1D
...
class GatePiPiCo2():
 ...
 #       self.roiState = 0
    ...
    def sum_under_2d_roi(self):
      #  print 'sum_2d i:',i
        if self.parent.RectGate2_roiState== 0:
            return 0
       ...
       # return np.sum(np.sum(arr))*scale_arr, coords_border_y_x1, coords_border_y_x2
        self.parent.PiPiCoUi.Rect2Yield.setText(self.roiName+"Yield:%0.1f" % roi_yield)
...
#        print self.XYG_ind1, self.XYG_ind2 
    ...
    def initialize_plots(self):
        ...
        self.plotWindow = PiPiCoGPlots2()   
        self.plot1dWindow = PiPiCoGPlots1D2()           
...
class PiPiCoGPlots1D2(QtGui.QMainWindow):
    def __init__(self, parent=None):

        super(PiPiCoGPlots1D2, self).__init__()
        
        
        self.PiPiCoGPlots1DUi = PiPiCo2GatedPlots1D.Ui_PiPiCo2GatedPlots1D()
        ...
        self.Plots1DCurve = self.Plots1DPlotWidget.plot(pen='g')
        ...
        self.Plots1DCurve1 = self.Plots1DPlotWidget1.plot(pen='g')
        self.Plots1DPlotWidget1.setLabel('left', 'Yield')# units='A')
        self.Plots1DPlotWidget1.setLabel('bottom', 'Radius (pixel)')                 
        
        self.show()

class PiPiCoGPlots2(QtGui.QMainWindow):
    def __init__(self, parent=None):

        super(PiPiCoGPlots2, self).__init__()
        
        self.pos, self.colors = zip(*cmapToColormap(matplotlib.cm.jet))
        # Set the colormap
        self.clrmp =  pg.ColorMap(self.pos, self.colors)
        
        self.PiPiCoGPlotsUi = PiPiCo2GatedPlots.Ui_PiPiCo2GatedPlots()
        self.PiPiCoGPlotsUi.setupUi(self)
        
        self.PiPiCoItem = pg.PlotItem(title = 'PiPiCo2',labels={'bottom': ('T', 'ns'), 'left': ('T', 'ns')})
        self.PiPiCoItem.setAspectLocked(False)                      
        self.PiPiCoPlot = pg.ImageView(name='PiPiCo2', view=self.PiPiCoItem) 
